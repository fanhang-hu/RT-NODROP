...
本测试方案验证安全感知任务调度复现实验是否满足比赛交付要求，重点确认：

1. 代码可以在指定环境中编译或运行。
2. 三类实验入口可复现输出结果。
3. 输出指标覆盖功能任务可调度性与安全任务完整性。
4. 正常、异常和边界场景均有明确测试方法。
5. 测试过程可被评审者根据仓库代码独立复现。

## 2. 测试范围

测试对象包括：

- `rta/safety_aware_sched.py`：比赛复现实验入口。
...
- 历史草稿脚本 `util_syscall_cpu_v*`、`check_util_*` 的完整回归。
- OpenHarmony 内核真实部署。当前仓库提供的是算法与实验复现代码，可作为迁移到 OpenHarmony 调度/安全子系统的参考实现。
- 论文 PDF 的排版或图形重绘。
...
- 论文 Fig. 4 的最终图形展示可由输出 `DATA` 段导入 gnuplot、matplotlib 或表格工具绘制，本测试方案只验证数据复现。

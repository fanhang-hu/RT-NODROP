...
现有处理方式往往偏向其中一个目标，如下图所示：集中式 consumer 易在高事件速率下形成跨任务、跨核干扰；同步处理能缩短事件等待，却会把额外开销直接放到系统调用路径上；按事件积压再批量处理虽然节省唤醒次数，却难以给处理时长和事件等待时间设定稳定边界。

![arch](/Users/hfh/Downloads/arch.png)
